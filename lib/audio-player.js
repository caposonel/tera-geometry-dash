...
//   MUSIC|<vol>|<loop:0|1>|<path>   open + play music
//   STOPMUSIC / PAUSE / RESUME      music control
//   VOL|<vol>                       music volume
//   SFX|<vol>|<path>                fire-and-forget effect
...
:main while ($true) {
    if ($task.IsCompleted) {
        ...
        switch ($parts[0]) {
            'MUSIC' {
                ...
                $music.Open([Uri]::new($parts[3]))
                $music.Play()
                $musicOpen = $true
            }
            'STOPMUSIC' { $music.Stop(); $music.Close(); $musicOpen = $false }
            'PAUSE'     { $music.Pause() }
            'RESUME'    { if ($musicOpen) { $music.Play() } }
            ...
            'STOPALL' {
                $music.Stop(); $music.Close(); $musicOpen = $false
    ...
    if ($musicLoop -and $musicOpen -and $music.NaturalDuration.HasTimeSpan) {
...
class AudioPlayer {
    ...
    _resolveMusic(key) {
        ...
        return path.join(this._assetsDir, 'music', fileName + '.mp3')
    }
    ...
    playMusic(key, loop) {
        ...
        this._musicPaused = false
        this._send('MUSIC|' + this._volume + '|' + (loop !== false ? '1' : '0') + '|' + filePath)
    }
    ...
    playEffect(key, volume) {
        ...
        if (!filePath || !fs.existsSync(filePath)) return
        const vol = Math.min(1, Math.max(0, volume != null ? Number(volume) || 0 : 0.8)) * this._volume
        this._send('SFX|' + vol + '|' + filePath)
    }
